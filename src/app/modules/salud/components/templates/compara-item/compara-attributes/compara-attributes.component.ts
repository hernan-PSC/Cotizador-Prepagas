...
export class ComparaAttributesComponent implements OnInit {
  ...
  findAttributeValue(producto: any, attributeName: string): string {
    ...
    // cosole.log('compara-attributes 38 return attribute ? attribute.value_name : ')
    // cosole.log('compara attributes 39 producto:')
    if(producto){// cosole.log(producto)
      }
    // cosole.log('compara attributes 41 attributeName:')
    if(attributeName){// cosole.log(attributeName)
      }
    // cosole.log('compara attributes 43 attribute.value_name:')
    if(attribute.value_name){// cosole.log(attribute.value_name)
      }
