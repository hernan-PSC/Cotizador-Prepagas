...
import {FormGroup, FormBuilder, Validators, FormsModule, ReactiveFormsModule,FormGroupDirective } from '@angular/forms';
...
export class FormularioComponent {
  ...
  @ViewChild('mainForm') mainFormDirective: FormGroupDirective;
  @ViewChild('formCotiza') formCotiza!: MatStepper;
  visible = false;   
  checkedmon: boolean;
  checkedaf: boolean;
 ...
 showCantidadHijos:boolean = false;
  ...
  //  isStepperVisible: boolean = true;
   ...
   verCotizacion() {
    console.log("this.isCotizacionVisible",this.isCotizacionVisible);
    console.log("this.selectedContactoType ",this.selectedContactoType)
...
goPrevious() {
  this.formCotiza.previous();
}
goNext() {
  this.formCotiza.next();
}
resetStepper() {
  this.formCotiza.reset();
}
...
console.log('Estado del formulario: reset', this.formCotizar.status);
      console.log('Formulario en metodo reset ',this.formCotizar.value); 
      }
 ...
 save(event: any){
   ...
   if(this.formCotizar.valid){
     console.log('Formulario en metodo save ',this.formCotizar.value);
 
     this.retornarService.setFormularioData(this.formCotizar.value)
 
     console.log('Formulario en metodo save2 ', this.formCotizar.value);
   } else {
...
this.retornarService.disparadorDePrecio.emit(this.formCotizar.value);
 ...
 // Example if you need to manually trigger submission logic from TypeScript:
    submitFormManually() {
        if (this.mainFormDirective) {
            // Check validation, show errors, then call save() if valid
            if (this.mainFormDirective.form.valid) {
              console.log("submitFormManually ");
                this.save(null); // Call your existing save method
                console.log("submitFormManually ");
            } else {
                this.mainFormDirective.form.markAllAsTouched();
...
selectContactoType(type: string) {
    ...
    console.log(type);
   this.formCotizar.get('personalData.medioContacto')?.setValue(type)
...
ngAfterViewInit(): void {
        // You can now access form properties after the view has initialized
        // console.log(this.mainFormDirective.form.value);
    }
